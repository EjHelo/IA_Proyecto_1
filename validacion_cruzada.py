...
def obtener_resultados(training_set, validation_set, modelo): 
	result_training = []
  	result_validation = []
...
def k_fold_cross_validation(k_validaciones, porcentaje_pruebas, examples, modelo):
	fold_error_t = 0
  	fold_error_v = 0
	...
	#-------------------------------------------------------
  	...
  	for i in range(k_validaciones):
    	...
    	result_training, result_validation = obtener_resultados(training_set, validation_set, modelo)

    	#Si es una red (NN) entonces ya en result_training y result_validation tengo el error rate
    	if modelo.NN:
  	...
  	result_training, result_validation = obtener_resultados(k_fold_examples, test_set, modelo)
